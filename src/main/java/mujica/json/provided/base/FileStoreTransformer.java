...
public class FileStoreTransformer implements JsonContextTransformer<FileStore> {
    ...
    @Override
    public void transform(@NotNull FileStore fileStore, @NotNull JsonHandler out, @Nullable JsonContext context) {
        out.openObject();
        {
            out.key(ClassLoaderTransformer.NAME);
            out.stringValue(fileStore.name());
            out.key(TYPE);
            out.stringValue(fileStore.type());
            out.key(READ_ONLY);
            out.booleanValue(fileStore.isReadOnly());
            try {
                long space = fileStore.getTotalSpace(); // throws IOException
                out.key(TOTAL);
                out.numberValue(space);
                space = fileStore.getUsableSpace(); // throws IOException
                out.key(USABLE);
                out.numberValue(space);
                space = fileStore.getUnallocatedSpace(); // throws IOException
                out.key(UNALLOCATED);
                out.numberValue(space);
                space = fileStore.getBlockSize(); // throws IOException
                out.key(BLOCK_SIZE);
                out.numberValue(space);
            } catch (Exception e) {
                if (context != null) {
                    context.getLogger().debug("{}", fileStore, e);
                }
            }
            try { // found in WindowsFileStore
                int vsn = (Integer) fileStore.getAttribute("volume:vsn"); // throws IOException, UnsupportedOperationException
                out.key("VSN");
                out.numberValue(vsn);
                boolean bool = (Boolean) fileStore.getAttribute("volume:isRemovable"); // throws IOException, UnsupportedOperationException
                out.key("removable");
                out.booleanValue(bool);
                bool = (Boolean) fileStore.getAttribute("volume:isCdrom"); // throws IOException, UnsupportedOperationException
                out.key("CDROM");
                out.booleanValue(bool);
            } catch (Exception e) {
                if (context != null) {
