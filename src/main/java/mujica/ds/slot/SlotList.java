...
@DirectSubclass({CopyOnResizeSlotList.class, CopyOnWriteSlotList.class})
public abstract class SlotList<S, A> implements SlotCollection<S, A> {

    protected abstract SlotArrayAllocator<S, A> getAllocator();

    @Override
    @NotNull
    public abstract SlotList<S, A> duplicate();
    ...
    @Override
    public void insertFirst(@NotNull S in) {
        insertItemBefore(0, in);
    }

    @Override
    public void insertLast(@NotNull S in) {
        insertItemBefore(size(), in);
    }

    @Override
    public void insertLast(@NotNull SlotCollection<S, A> that) {
        that.forEach(this::insertLast, getAllocator().newSlot());
    }

    @Override
    public void insertLast(@NotNull A inArray, int startIndex, int endIndex) {
        final SlotArrayAllocator<S, A> allocator = getAllocator();
        final S tempSlot = allocator.newSlot();
        for (int index = startIndex; index < endIndex; index++) {
            allocator.load(inArray, index, tempSlot);
            insertLast(tempSlot);
        }
    }
    ...
    public long sort(@NotNull Sort<A> sort) {
        ...
        final int thisSize = allocator.length(tempArray);
        final long operationCount = sort.sort(tempArray, 0, thisSize);
        final S tempSlot = allocator.newSlot();
    ...
    public long sortPart(@NotNull Sort<A> sort, int newEndIndex) {
        ...
        final long operationCount = sort.sortPart(tempArray, 0, newEndIndex, thisSize);
        final S tempSlot = allocator.newSlot();
    ...
    public long sortUnique(@NotNull Sort<A> sort) {
        ...
        final I32 endSlot = new I32(thisSize);
        final long operationCount = sort.sortUnique(tempArray, 0, endSlot);
    ...
    public GapIterator<S> iterator(int startGapIndex) {
        ...
        return new GapIterator<>() {
            ...
            @Override
            public void removePrevious(@NotNull S out) throws NoSuchElementException {
                try {
            ...
            @Override
            public void removeNext(@NotNull S out) throws NoSuchElementException {
                try {
