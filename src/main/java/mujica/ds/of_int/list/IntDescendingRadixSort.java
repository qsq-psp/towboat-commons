...
@CodeHistory(date = "2026/2/5")
@ReferencePage(title = "基数排序", href = "https://oi-wiki.org/basic/radix-sort/")
public class IntDescendingRadixSort extends SortingAlgorithm<int[]> {
    ...
    public IntDescendingRadixSort(int modulo) {
        super();
        if (modulo < 3) {
    ...
    public long sort(@NotNull int[] array, int startIndex, int endIndex) {
        ...
        while (true) {
            ...
            for (int index = startIndex; index < endIndex; index++) {
                ...
                bucketNumber = moduloM1 - bucketNumber;
            ...
            if (((long) max) - min < modulo) {
                ...
                if (min < max) {
                    min++;
                    System.arraycopy(buckets0, 0, buckets1, modulo - min, min);
                    System.arraycopy(buckets0, min, buckets1, 0, modulo - min);
