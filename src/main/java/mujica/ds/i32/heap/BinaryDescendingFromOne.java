...
@CodeHistory(date = "2024/1/15", project = "Ultramarine", name = "IntHeap.MaxS1")
@CodeHistory(date = "2026/1/10")
public class BinaryDescendingFromOne extends ArrayIntHeap {
    ...
    public BinaryDescendingFromOne(@Nullable CapacityPolicy policy) {
        super(policy);
    }

    BinaryDescendingFromOne(@NotNull CapacityPolicy policy, @NotNull int[] array) {
        super(policy, array);
    }

    @NotNull
    @Override
    public BinaryDescendingFromOne duplicate() {
        final BinaryDescendingFromOne that = new BinaryDescendingFromOne(
    ...
    public void checkHealth(@NotNull Consumer<RuntimeException> consumer) {
        ...
        for (int i = 2; i < size; i++) { // child
            int j = i >> 1; // parent
            if (array[i] > array[j]) {
                consumer.accept(new InvariantException("array[" + i + "] = " + array[i] + " (as child) bigger than array[" + j + "] = " + array[j] + " (as parent)"));
            }
        }
    }
