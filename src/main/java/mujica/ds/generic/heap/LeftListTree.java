...
@CodeHistory(date = "2025/5/30", project = "Ultramarine", name = "LeftListHeap")
@CodeHistory(date = "2025/6/3")
@Name(value = "左偏树", language = "zh")
public class LeftListTree<E> extends BiasedPriorityQueue<E> {

    public LeftListTree(@Nullable Comparator<E> comparator) {
        super(comparator);
    }

    @NotNull
    @Override
    public LeftListTree<E> duplicate(@NotNull UnaryOperator<E> operator) {
        final LeftListTree<E> that = new LeftListTree<>(comparator);
        if (this.root != null) {
    ...
    public void checkHealth(@NotNull Consumer<RuntimeException> consumer) {
        int expectedSize = 0;
        if (root != null) {
            expectedSize = root.checkLeftListHealth(comparator, consumer);
        }
    ...
    private BiasedNode<E> merge(@Nullable BiasedNode<E> t0, @Nullable BiasedNode<E> t1) {
        ...
        t0.shallowChild = merge(t0.shallowChild, t1);
        t0.balanceNode();
        return t0;
    }
