 ...
 */
@Name(value = "计数排序", language = "zh")
public class IntAscendingCountSort extends SortingAlgorithm<int[]> {

    private final int[] count = new int[1 << Byte.SIZE];

    private transient int[] shared;

    public IntAscendingCountSort(boolean hasShared) {
        super();
        if (hasShared) {
    ...
    public long sort(@NotNull int[] target, int startIndex, int endIndex) {
        ...
        for (int shift = 0; shift < Integer.SIZE; shift += Byte.SIZE) {
            ...
            {
                int sum = 0;
                for (int masked = 0; masked < (1 << Byte.SIZE); masked++) {
                    int oldSum = sum;
                    sum += count[masked];
                    count[masked] = oldSum;
                }
