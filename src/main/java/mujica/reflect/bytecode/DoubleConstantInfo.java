package mujica.reflect.bytecode;

import mujica.io.nest.LimitedDataInput;
import mujica.reflect.modifier.CodeHistory;
import mujica.reflect.modifier.ReferencePage;
import org.jetbrains.annotations.NotNull;
import org.jetbrains.annotations.Nullable;

import java.io.DataOutput;
import java.io.IOException;
import java.nio.ByteBuffer;

@CodeHistory(date = "2019/8/15", project = "bone", name = "JavaConstantDouble")
@CodeHistory(date = "2025/9/20")
@ReferencePage(title = "JVMS12 The CONSTANT_Long_info and CONSTANT_Double_info Structures", href = "https://docs.oracle.com/javase/specs/jvms/se12/html/jvms-4.html#jvms-4.4.5")
class DoubleConstantInfo extends ConstantInfo implements ConstantPool.Large {

    private static final long serialVersionUID = 0xf8c5bd1075c263d0L;

    double value;

    DoubleConstantInfo() {
        super();
    }

    @Override
    protected int tag() {
        return ConstantPool.CONSTANT_DOUBLE;
    }

    protected int section() {
        return 5;
    }

    protected int sinceVersion() {
        return (45 << Short.SIZE) | 3;
    }

    @Nullable
    protected Class<?> loadedClass() {
        return double.class;
    }

    @Override
    public void read(@NotNull LimitedDataInput in) throws IOException {
        value = in.readDouble();
    }

    @Override
    public void read(@NotNull ByteBuffer buffer) {
        value = buffer.getDouble();
    }

    @Override
    public void write(@NotNull DataOutput out) throws IOException {
        out.writeDouble(value);
    }

    @Override
    public void write(@NotNull ByteBuffer buffer) {
        buffer.putDouble(value);
    }

    @NotNull
    @Override
    public String toString(@NotNull ClassFile context) {
        return "#" + newIndex + " double " + value;
    }

    @NotNull
    @Override
    protected String constantValueToString(@NotNull ClassFile context) {
        if (Double.isNaN(value)) {
            return "Double.NaN";
        } else if (value == Double.POSITIVE_INFINITY) {
            return "Double.POSITIVE_INFINITY";
        } else if (value == Double.NEGATIVE_INFINITY) {
            return "Double.NEGATIVE_INFINITY";
        } else {
            return value + "D";
        }
    }

    @Override
    public void retain(@NotNull ClassFile context) {
        referenceCount++;
    }

    @Override
    public int hashCode() {
        return Double.hashCode(value);
    }

    @Override
    public boolean equals(Object obj) {
        return obj instanceof DoubleConstantInfo && Double.doubleToRawLongBits(value) == Double.doubleToRawLongBits(((DoubleConstantInfo) obj).value);
    }
}
