...
public class SourceMethodType implements NotLoadedMethodType<SourceFieldType, SourceMethodType> {
    ...
    public BytecodeMethodType toBytecodeMethodType() {
        return new BytecodeMethodType(toBytecodeString());
    }
    ...
    public int hashCode() {
        ...
        for (SourceFieldType parameterType : parameterTypes) {
            hash = hash * 59 + parameterType.hashCode();
        }
        return hash;
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof SourceMethodType)) {
            return false;
        }
        final SourceMethodType that = (SourceMethodType) obj;
        if (!this.returnType.equals(that.returnType)) {
            return false;
        }
        final int length = this.parameterTypes.length;
        if (length != that.parameterTypes.length) {
            return false;
        }
        for (int index = 0; index < length; index++) {
            if (!this.parameterTypes[index].equals(that.parameterTypes[index])) {
                return false;
            }
        }
        return true;
