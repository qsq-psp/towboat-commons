...
public class RandomContext implements Serializable {
    ...
    public boolean nextBoolean() {
        return source.applyAsLong(1) != 0L;
    }

    public byte nextByte() {
        return (byte) source.applyAsLong(Byte.SIZE);
    }
    ...
    public short nextShort() {
        return (short) source.applyAsLong(Short.SIZE);
    }
    ...
    public int nextInt() {
        return (int) source.applyAsLong(Integer.SIZE);
    }

    public int nextNonNegativeInt() {
        return (int) source.applyAsLong(Integer.SIZE - 1);
    }
    ...
    public int nextBinomial(int n, double p) {
        ...
        if (Math.abs(p - 0.5) < EPS) {
            ...
            if (n > 0) {
                x += Long.bitCount(source.applyAsLong(n));
            }
        } else {
            while (n > 0) {
                if (nextDouble() < p) {
    ...
    public long nextLong() {
        return source.applyAsLong(Long.SIZE);
    }

    public long nextNonNegativeLong() {
        return source.applyAsLong(Long.SIZE - 1);
    }
    ...
    private int nextFloatMantissa() {
        return (int) source.applyAsLong(24);
    }
    ...
    public BigInteger nextBigInteger(int bitLength) {
        ...
        for (int index = 0; index < byteCount; index++) {
            bytes[index] = (byte) source.applyAsLong(Byte.SIZE); // nextByteArray(int length) is override, so put it inline
        }
