...
public class DataSizeAppender extends HexEncoder implements IntegralToStringFunction {
    ...
    protected DataSizeAppender(@MagicConstant(valuesFromClass = Base16Case.class) int alphabetOffset, int style, int mantissa, @NotNull String itemDelimiter) {
    ...
    @NotNull
    public static DataSizeAppender create(boolean upperCase, int style, int mantissa, @Nullable String itemDelimiter) {
        return new DataSizeAppender(
                upperCase ? UPPER_CONSTANT : LOWER_CONSTANT,
                style, mantissa,
    ...
    @NotNull
    public static DataSizeAppender createDec() {
        return new DataSizeAppender(LOWER_CONSTANT, STYLE_DEC, 0, "");
    }

    @NotNull
    public static DataSizeAppender createHex(boolean upperCase, boolean padding) {
        int style = STYLE_HEX;
        if (padding) {
            style |= HEX_PADDING;
        }
        return new DataSizeAppender(upperCase ? UPPER_CONSTANT : LOWER_CONSTANT, style, 0, "");
    }

    @NotNull
    public static DataSizeAppender create1000(int mantissa) {
        return new DataSizeAppender(LOWER_CONSTANT, STYLE_1000, mantissa, "");
    }

    @NotNull
    public static DataSizeAppender create1024(int mantissa) {
        return new DataSizeAppender(LOWER_CONSTANT, STYLE_1024, mantissa, "");
    }
    ...
    private void appendMantissa(long numerator, long denominator, @NotNull StringBuilder out) {
        ...
        try {
            ...
            out.append('.');
            mantissa.acceptLong(numerator, out);
    ...
    private void appendMantissa(BigInteger numerator, BigInteger denominator, @NotNull StringBuilder out) {
        ...
        out.append('.');
        mantissa.acceptBig(numerator, out);
    }
    ...
    public void append(@NotNull BigInteger value, @NotNull StringBuilder out) {
        ...
        if ((style & STYLE_HEX) != 0) {
            ...
            if (alphabetOffset == UPPER_CONSTANT) {
