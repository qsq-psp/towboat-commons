...
import mujica.ds.any.ComparableComparator;
...
public class ListSortingAlgorithmTest {
    ...
    public void checkObject() {
        // checkObject(InsertionSort::new);
        // checkObject(MergeSort::new);
        // checkObject(t -> new SelectionSort(comparator));
    }

    private void checkString(@NotNull SortingAlgorithm<List<String>> algorithm) {
        Assert.assertEquals(MonotonicityDirection.ASCENDING, algorithm.monotonicity());
        final HashMap<String, S32> counter = new HashMap<>();
        for (int repeatIndex = 0; repeatIndex < REPEAT_COUNT; repeatIndex++) {
            counter.clear();
            int prefixLength = rc.nextInt(ASIDE_LENGTH);
            int prefixAndMainLength = prefixLength + rc.nextInt(2, MAIN_LENGTH);
            ...
            try {
                ...
                for (String string : list) {
                    S32.decrease(counter, string);
                }
                for (S32 value : counter.values()) {
                    Assert.assertEquals(0, value.getI32());
                }
            } catch (AssertionError e) {
                LOGGER.error("{}", list);
                throw e;
    ...
    public void checkString() {
        ...
        // checkString(new InsertionSort<>(comparator));
        // checkString(new MergeSort<>(comparator));
        // checkString(new SelectionSort<>(comparator));
    }
    ...
    private void checkTimeUnit(@NotNull SortingAlgorithm<List<TimeUnit>> algorithm) {
        Assert.assertEquals(MonotonicityDirection.ASCENDING, algorithm.monotonicity());
        final HashMap<TimeUnit, S32> counter = new HashMap<>();
        for (int repeatIndex = 0; repeatIndex < REPEAT_COUNT; repeatIndex++) {
            counter.clear();
            int prefixLength = rc.nextInt(ASIDE_LENGTH);
            int prefixAndMainLength = prefixLength + rc.nextInt(2, MAIN_LENGTH);
    ...
    public void checkTimeUnit() {
        // checkTimeUnit(new InsertionSort<>(TIME_UNIT_COMPARATOR));
        // checkTimeUnit(new MergeSort<>(TIME_UNIT_COMPARATOR));
        // checkTimeUnit(new SelectionSort<>(TIME_UNIT_COMPARATOR));
    }
    ...
    private void checkTimeZone(@NotNull SortingAlgorithm<List<TimeZone>> algorithm) {
        Assert.assertEquals(MonotonicityDirection.ASCENDING, algorithm.monotonicity());
        final HashMap<TimeZone, S32> counter = new HashMap<>();
        for (int repeatIndex = 0; repeatIndex < REPEAT_COUNT; repeatIndex++) {
            counter.clear();
            int prefixLength = rc.nextInt(ASIDE_LENGTH);
    ...
    public void checkTimeZone() {
        // checkTimeZone(new InsertionSort<>(TIME_ZONE_COMPARATOR));
        // checkTimeZone(new MergeSort<>(TIME_ZONE_COMPARATOR));
        // checkTimeZone(new SelectionSort<>(TIME_ZONE_COMPARATOR));
    }
    ...
    private void checkLocale(@NotNull SortingAlgorithm<List<Locale>> algorithm) {
        Assert.assertEquals(MonotonicityDirection.ASCENDING, algorithm.monotonicity());
        final HashMap<Locale, S32> counter = new HashMap<>();
        for (int repeatIndex = 0; repeatIndex < REPEAT_COUNT; repeatIndex++) {
            counter.clear();
            List<Locale> list = nextLocaleList();
            for (Locale locale : list) {
                S32.increase(counter, locale);
            }
            algorithm.sort(list);
            try {
    ...
    public void checkLocale() {
        // checkLocale(new InsertionSort<>(LOCALE_COMPARATOR));
        // checkLocale(new MergeSort<>(LOCALE_COMPARATOR));
        // checkLocale(new SelectionSort<>(LOCALE_COMPARATOR));
    }
}
